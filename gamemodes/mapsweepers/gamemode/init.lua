...
-- // ConVars {{{
	...
	jcms.cvar_ffmul = CreateConVar("jcms_friendlyfire_multiplier", "1", FCVAR_JCMS_NOTIFY_AND_SAVE, "Friendly fire damage is multiplied by this number. 0 disables friendly fire. This applies to turrets and orbitals!", 0, 100)

	jcms.cvar_map_excludecurrent = CreateConVar("jcms_map_excludecurrent", "0", FCVAR_JCMS_NOTIFY_AND_SAVE, "Excludes the current server map from the post-mission vote, ensuring that every mission is on a new map. Unless there's no other valid map.")
	jcms.cvar_map_iswhitelist = CreateConVar("jcms_map_iswhitelist", "0", FCVAR_JCMS_NOTIFY_AND_SAVE, "Alters the behaviour of jcms_map_list. If this is 1, the list of maps will be a 'whitelist' (ONLY those maps will be picked). If this is 0, the list of maps will be a 'blacklist' (those maps will be EXCLUDED)")
	...
	-- Replicated
	jcms.cvar_noepisodes = CreateConVar("jcms_noepisodes", "0", bit.bor(FCVAR_REPLICATED, FCVAR_JCMS_NOTIFY_AND_SAVE), "If set to 1, Half-Life 2: Episode One & Two content will never appear in-game. Useful if you don't want your poor friends to see errors.")
